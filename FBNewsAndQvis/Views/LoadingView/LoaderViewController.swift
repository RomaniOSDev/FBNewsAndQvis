...
class LoadingSplash: UIViewController {
    ...
    private func setupUI() {
        ...
        loadingImage.image = UIImage(resource: .cupWin)
    ...
    func generateTrackingLink() -> String {
        let base = "https://penaltylegends.store/6kh1J875"
        if let savedURL = UserDefaults.standard.string(forKey: "finalAppsflyerURL") {
...
extension AppDelegate: AppsFlyerLibDelegate {
    func onConversionDataSuccess(_ conversionInfo: [AnyHashable: Any]) {
        ...
        // Проверяем organic статус
        let isOrganic = (data["af_status"] as? String)?.lowercased() == "organic"
        let campaign = isOrganic ? "organic" : (data["campaign"] as? String ?? "")
        
        let sub1 = data["af_sub1"] as? String ?? ""
        let sub2 = data["af_ad_type"] as? String ?? ""
        let sub3 = data["af_cost_value"] as? String ?? ""
        let sub4 = data["af_cost_currency"] as? String ?? ""
        let sub5 = data["af_ad_id"] as? String ?? ""
        let sub6 = data["af_siteid"] as? String ?? ""
